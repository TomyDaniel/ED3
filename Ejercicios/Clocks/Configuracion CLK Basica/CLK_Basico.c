...
void configuraciones(void){
    ...
    LPC_TIM0->TCR = 0; // me aseguro que el timer este deshabilitado

    // configurar el Prescaler.
    // la idea es que el contador del timer se incremente cada 1 us.
    // frecuencia del PCLK = 25,000,000 Hz.
    // prescaler = (PCLK / FrecuenciaDeseada) - 1
    // prescaler = (25,000,000 / 1,000,000) - 1 = 24.
    LPC_TIM0->PR = 24;

    //esto es para resetear el contador
    LPC_TIM0->TCR |= (1 << 1);
    LPC_TIM0->TCR &= ~(1 << 1);
